...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initializeAudio() {
        try {
            const response = await fetch(`${serverUrl}/synthesize-speech`, { // 新しいエンドポイントを使用
                method: 'POST',
            ...
            displayError("初期ストーリーの音声再生に失敗しました。");
        }
    }
    ...
    // Web Audio APIのコンテキストを準備（ページ読み込み時に一度だけ作成）
    const audioContext = new (window.AudioContext || window.webkitAudioContext)();
